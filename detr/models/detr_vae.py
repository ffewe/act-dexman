...
class DETRVAE(nn.Module):
    ...
    def forward(self, qpos, image, force, env_state, actions=None, is_pad=None):
        ...
        if is_training:
            ...
            force_embed = self.force_proj(force)
            force_embed = torch.unsqueeze(force_embed, axis=1)

            encoder_input = torch.cat([cls_embed, force_embed, qpos_embed, action_embed], axis=1) # (bs, seq+1, hidden_dim)
        ...
        if self.backbones is not None:
            ...
            force_input=self.input_proj_force(force)
            # print(torch.cat([proprio_input, force_input],axis=1).shape)
            # final_input=torch.cat([proprio_input,force_input],axis=0)

            # fold camera dimension into width dimension
            src = torch.cat(all_cam_features, axis=3)
            pos = torch.cat(all_cam_pos, axis=3)
            hs = self.transformer(src, None, self.query_embed.weight, pos, latent_input, proprio_input, force_input, self.additional_pos_embed.weight)[0]
        else:
